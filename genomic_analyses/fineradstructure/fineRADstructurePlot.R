...
setwd("/dss/dsslegfs01/pr53da/pr53da-dss-0029/diana/paper_final_datasets/fineradstruct/scripts/") ## The directory where the files are located
chunkfile<-"Alps_dsB_paper_chunks.out" ## RADpainter output file
mcmcfile<-"Alps_dsB_paper_chunks.mcmc.xml" ## finestructure mcmc file
treefile<-"Alps_dsB_paper_chunks.mcmcTree.xml" ## finestructure tree file
### 2) EDIT THIS PATH TO WHERE YOU WANT THE PLOTS:
plotsFolder <- "/dss/dsslegfs01/pr53da/pr53da-dss-0029/diana/paper_final_datasets/fineradstruct/"
### 3) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
analysisName <- "try_Alps_dsB_paper_frs";  maxIndv <- 10000; maxPop<-10000


### 4) EDIT THE PATH TO YOUR COPY of FinestructureLibrary.R
source("/dss/dsslegfs01/pr53da/pr53da-dss-0029/diana/paper_final_datasets/fineradstruct/scripts/FinestructureLibrary.R", chdir = TRUE) # read in the R functions, which also calls the needed packages
